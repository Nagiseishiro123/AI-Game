...
class MinesweeperAI():
    ...
    def make_safe_move(self):
     """
     Returns a safe cell to choose on the Minesweeper board using BrFS.
     """
     # Tạo một hàng đợi để lưu trữ các ô an toàn
     safe_queue = deque()
    
     # Thêm tất cả các ô an toàn hiện tại vào hàng đợi
     for cell in self.safes:
        if cell not in self.moves_made:
            safe_queue.append(cell)
    
     # Thực hiện BFS để tìm ô an toàn gần nhất
     while safe_queue:
        cell = safe_queue.popleft()
        
        # Nếu ô chưa được mở, đó sẽ là nước đi an toàn
        if cell not in self.moves_made:
            return cell
        
        # Thêm các ô an toàn lân cận vào hàng đợi
        for neighbor in self.nearby_cells(cell):
            if neighbor not in self.moves_made and neighbor not in self.mines:
                safe_queue.append(neighbor)
    
    # Không tìm thấy nước đi an toàn
     return None      
